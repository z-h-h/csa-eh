...
public:
  ...
  LLVM_DUMP_METHOD static StringRef getKindAsString(ItemKind K) {
    switch (K) {
      ...
      case ThrowKind:               return "construct into throw variable";
...
public:
  ...
  ConstructionContextItem(const CXXThrowExpr *TE)
      : Data(TE), Kind(ThrowKind) {}
...
public:
  enum Kind {
    ...
    SimpleThrownValueKind,
    // CXX17ElidedCopyThrownValueKind,
    THROWN_VALUE_BEGIN = SimpleThrownValueKind,
    THROWN_VALUE_END = SimpleThrownValueKind, // CXX17ElidedCopyThrownValueKind
    ArgumentKind
  };
...
class ThrownValueConstructionContext : public ConstructionContext {
  const CXXThrowExpr *TE;

protected:
  explicit ThrownValueConstructionContext(ConstructionContext::Kind K,
                                          const CXXThrowExpr *TE)
      : ConstructionContext(K), TE(TE) {
    assert(classof(this));
    assert(TE);
  }

public:
  const CXXThrowExpr *getCXXThrowExpr() const { return TE; }

  static bool classof(const ConstructionContext *CC) {
    return CC->getKind() >= THROWN_VALUE_BEGIN &&
           CC->getKind() <= THROWN_VALUE_END;
  }
};

class SimpleThrownValueConstructionContext
    : public ThrownValueConstructionContext {
  friend class ConstructionContext; // Allows to create<>() itself.

  explicit SimpleThrownValueConstructionContext(const CXXThrowExpr *TE)
      : ThrownValueConstructionContext(
            ConstructionContext::SimpleThrownValueKind, TE) {}

public:
  static bool classof(const ConstructionContext *CC) {
    return CC->getKind() == SimpleThrownValueKind;
  }
};
