...
protected:
  ...
  const CFGBlock *TryBlock = nullptr;
  size_t DtorID = 0;
...
public:
  CFGStmt(Stmt *S, Kind K, const CFGBlock *B, size_t ID) : CFGElement(K, S) {
    assert(isKind(*this));
    TryBlock = const_cast<CFGBlock *>(B);
    DtorID = ID;
  }
  ...
  /// When this statement throws an exception, the control flow should
  /// immediately jump to a try block to which this statement belongs.
  /// This function returns the try block & it should never be a nullptr.
  /// (Since there is always a pseudo try block)
  const CFGBlock *getTryBlock() const { return TryBlock; }

  /// When this statement throws an exception, the control flow should jump
  /// to a try block. But it does not execute from the begining of this try
  /// block. Instead, it starts at a certain destructor in this try block.
  /// This function returns the index of the destructor. The index may be -1
  /// if there is no destructor OR the destrcutor is NOT in this try block.
  /// -1 indicates the control flow to jump to the end of the try block.
  int getDtorIdx() const;
...
public:
  explicit CFGConstructor(CXXConstructExpr *CE, const ConstructionContext *C,
                          const CFGBlock *B, size_t ID)
      : CFGStmt(CE, Constructor, B, ID) {
    assert(C);
...
public:
  ...
  explicit CFGCXXRecordTypedCall(Expr *E, const ConstructionContext *C,
                                 const CFGBlock *B, size_t ID)
      : CFGStmt(E, CXXRecordTypedCall, B, ID) {
...
public:
  ...
  CFGAutomaticObjDtor(const VarDecl *var, const Stmt *stmt, const CFGBlock *B, size_t ID)
      : CFGImplicitDtor(AutomaticObjectDtor, var, stmt) {
    TryBlock = B;
    DtorID = ID;
  }
  ...
  /// Get the unique id of this destructor. Two destructors are same if and only
  /// if they have the same ids.
  size_t getUniqueID() const { return reinterpret_cast<size_t>(getVarDecl()); }

  /// Return whether this destructor is an exceptional destrcutor (in try block)
  /// or not. (normal destructor)
  bool isExceptionalDtor() const { return TryBlock; }

  /// If this destructor is an exceptional control flow destructor, it should
  /// have a next destructor. (except the first var in the whole function)
  /// This function returns an index of the next destructor in this
  /// destructor's own try block. -1 indicates no next destructor OR the next
  /// destructor is NOT in the same try block as this destructor's.
  int getNextDtorIdx() const;
...
private:
  ...
  const CFGBlock *TryBlock;
  size_t NextDtorID;
public:
  CFGTerminator() { assert(!isValid()); }
  CFGTerminator(Stmt *S, Kind K = StmtBranch) : Data(S, K) {}

  CFGTerminator(CXXTryStmt *S, const CFGBlock *B, size_t ID)
      : Data(S, StmtBranch), TryBlock(B), NextDtorID(ID) {}
...
public:
  ...
  const std::deque<const CXXCatchStmt *> &getCatchStmts() const {
    return CatchStmts;
  }

  void setCatchStmts(const std::deque<const CXXCatchStmt *> &CS) {
    CatchStmts = CS;
  }
  ...
  void appendStmt(Stmt *statement, BumpVectorContext &C, const CFGBlock *B,
                  size_t ID) {
    Elements.push_back(CFGStmt(statement, CFGElement::Statement, B, ID), C);
  }

  void appendConstructor(CXXConstructExpr *CE, const ConstructionContext *CC,
                         BumpVectorContext &C, const CFGBlock *B,
                         size_t ID) {
    Elements.push_back(CFGConstructor(CE, CC, B, ID), C);
  }

  void appendCXXRecordTypedCall(Expr *E, const ConstructionContext *CC,
                                BumpVectorContext &C, const CFGBlock *B,
                                size_t ID) {
    Elements.push_back(CFGCXXRecordTypedCall(E, CC, B, ID), C);
  }
  ...
  void appendAutomaticObjDtor(VarDecl *VD, Stmt *S, BumpVectorContext &C, size_t ID) {
    Elements.push_back(CFGAutomaticObjDtor(VD, S, this, ID), C);
  }
...
public:
  ...
  CFGBlock *initPseudoTryBlock();

  void finishPseudoTryBlock();
  ...
  /// Set pseudo try block. This is typically used only during CFG construction.
  void setPseudoTryBlock(CFGBlock *B) { PseudoTryBlock = B; }
  ...
  CFGBlock *getPseudoTryBlock() { return PseudoTryBlock; }
  const CFGBlock *getPseudoTryBlock() const { return PseudoTryBlock; }
  ...
  unsigned getNumUnmergedBlocks() const { return NumUnmergedBlocks; }
  unsigned getNumMergedBlocks() const { return NumMergedBlocks; }

  void calcMergedAndUnmergedBlocks();
  ...
  void print(raw_ostream &OS, const LangOptions &LO, bool ShowColors,
             bool showException) const;
  void dump(const LangOptions &LO, bool ShowColors) const;
  void dumpException(const LangOptions &LO, bool ShowColors) const;
...
private:
  ...
  /// A pseudo try block is not a real block, it serves as a top level
  /// try block which contains automatic object dtors. Control flow will
  /// eventually reach the pseudo top level try block during stack
  /// unwinding, handling object destructions. A pseudo try block contains
  /// no terminator (unlike the other normal try block), and its only
  /// successor is the exit block.
  CFGBlock *PseudoTryBlock = nullptr;

  unsigned  NumBlockIDs = 0;

  unsigned NumUnmergedBlocks = 0;
  unsigned NumMergedBlocks = 0;
