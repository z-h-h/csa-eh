...
// This region stores catch variable which is declared as
// a reference, e.g. catch (int &i). There is no suitable
// memory region for variable i, so there is CXXCaughtVarRegion.
// When binding a thrown value to i, the result will be
// VarDeclRegion -> CXXCaughtVarRegion
// CXXCaughtVarRegion -> ThrownVal
class CXXCaughtVarRegion : public VarRegion {
  friend class MemRegionManager;

  const VarDecl *CatchRefDecl;
  QualType PointeeType;

  CXXCaughtVarRegion(const VarDecl *D, const MemRegion *sReg)
      : VarRegion(sReg, CXXCaughtVarRegionKind), CatchRefDecl(D) {
    assert(D && isa<StackLocalsSpaceRegion>(sReg));
    const ReferenceType *RT =
        dyn_cast_or_null<ReferenceType>(D->getType().getTypePtr());
    assert(RT);
    PointeeType = RT->getPointeeType();
  }
  static void ProfileRegion(llvm::FoldingSetNodeID &ID, const VarDecl *D,
                            const MemRegion *sReg);

public:
  void Profile(llvm::FoldingSetNodeID &ID) const override;

  QualType getValueType() const override { return PointeeType; }

  const VarDecl *getDecl() const override { return CatchRefDecl; }
  ...
  static bool classof(const MemRegion *R) {
    return R->getKind() == CXXCaughtVarRegionKind;
  }
};
...
public:
  ...
  const CXXCaughtVarRegion *getCXXCaughtVarRegion(const VarDecl *VD,
                                                  const LocationContext *LC);
