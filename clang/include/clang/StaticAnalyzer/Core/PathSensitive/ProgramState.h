...
private:
  ...
  typedef std::pair<SVal, QualType> TypedVal;
  bool hasUncaught;                // Is there an uncaught exception?
  int tryBlockBeginIdx;            // Stores the begin index of try block
  std::deque<TypedVal> thrownVals; // Lifetime: throw -> catch begin
  std::deque<TypedVal> caughtVals; // Lifetime: catch begin -> catch end
  const CXXThrowExpr *CTE;         // The latest throw expr (Diagnostics only)
...
public:
  ...
  static void Profile(llvm::FoldingSetNodeID& ID, const ProgramState *V) {
    ...
    V->GDM.Profile(ID);
    ID.AddInteger(V->tryBlockBeginIdx);
    ID.AddPointer(V->CTE);
    ID.AddBoolean(V->hasUncaught);
    for (auto &Val : V->thrownVals) {
      // Why profile an additional integer?
      // e.g. after the throw statement:
      //    thrownVals: 123, 456, caughtVals: EMPTY
      // right after the catch statement:
      //    thrownVals: 123, caughtVals: 456
      // If only SVals are used to profile, then the two states are treated
      // as the same. If we add a bool (as a tag), then the profile will be
      // 1/123,1/456 & 1/123,0/456, thus they are different.
      ID.AddInteger(1);
      Val.first.Profile(ID);
      Val.second.Profile(ID);
    }
    for (auto &Val : V->getCaughtVals()) {
      ID.AddInteger(0);
      Val.first.Profile(ID);
      Val.second.Profile(ID);
    }
  ...
  LLVM_NODISCARD ProgramStateRef setUncaughtThrow(bool U) const;

  bool hasUncaughtThrow() const { return hasUncaught; }

  LLVM_NODISCARD ProgramStateRef pushThrownVal(SVal V, QualType T) const;

  const std::deque<TypedVal> &getThrownVals() const { return thrownVals; }

  LLVM_NODISCARD ProgramStateRef popThrownVal() const;

  LLVM_NODISCARD ProgramStateRef pushCaughtVal(SVal V, QualType T) const;

  LLVM_NODISCARD ProgramStateRef popCaughtVals(int Cnt) const;

  const std::deque<TypedVal> &getCaughtVals() const { return caughtVals; }

  LLVM_NODISCARD ProgramStateRef setTryBlockIdx(int Idx) const;

  int getTryBlockIdx() const { return tryBlockBeginIdx; }

  LLVM_NODISCARD ProgramStateRef setThrowExpr(const CXXThrowExpr *CTE) const;

  const CXXThrowExpr *getThrowExpr() const { return CTE; }
