...
namespace {
class ECFGDumper : public Checker<check::ASTCodeBody> {
public:
  void checkASTCodeBody(const Decl *D, AnalysisManager& mgr,
                        BugReporter &BR) const {
    PrintingPolicy Policy(mgr.getLangOpts());
    ...
    if (CFG *cfg = mgr.getCFG(D)) {
      cfg->dumpException(mgr.getLangOpts(),
                         llvm::sys::Process::StandardErrHasColors());
    }
  }
};
...
void ento::registerECFGDumper(CheckerManager &mgr) {
  mgr.registerChecker<ECFGDumper>();
}

bool ento::shouldRegisterECFGDumper(const CheckerManager &mgr) {
  return true;
}
