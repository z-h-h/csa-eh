...
private:
  CallDescriptionMap<FnDescription> FnDescriptions = {
      ...
      {{"open"}, {nullptr, &StreamChecker::evalOpen, ArgNone}},
      {{"socket", 3}, {nullptr, &StreamChecker::evalOpen, ArgNone}},
      {{"accept", 3}, {nullptr, &StreamChecker::evalOpen, ArgNone}},
      {{"connect", 3}, {nullptr, &StreamChecker::evalOpen, ArgNone}},
      {{"freopen", 3},
       {&StreamChecker::preFreopen, &StreamChecker::evalFreopen, 2}},
      {{"tmpfile"}, {nullptr, &StreamChecker::evalFopen, ArgNone}},
      {{"fclose", 1},
       {&StreamChecker::preDefault, &StreamChecker::evalFclose, 0}},
      {{"close", 1}, {nullptr, &StreamChecker::evalFclose, 0}},
      {{"shutdown", 2}, {nullptr, &StreamChecker::evalFclose, 0}},
      {{"fread", 4},
  ...
  void evalOpen(const FnDescription *Desc, const CallEvent &Call,
                CheckerContext &C) const;
...
void StreamChecker::evalOpen(const FnDescription *Desc, const CallEvent &Call,
                             CheckerContext &C) const {
  ProgramStateRef State = C.getState();
  const CallExpr *CE = dyn_cast_or_null<CallExpr>(Call.getOriginExpr());
  if (!CE)
  ...
  // Bifurcate the state into two: one with a valid fd, the other with a "-1".
  DefinedOrUnknownSVal Error = C.getSValBuilder().makeIntVal(-1, false);

  SVal OpenFailed =
      C.getSValBuilder().evalBinOp(State, BO_EQ, RetVal, Error, CE->getType());
  if (Optional<DefinedSVal> OpenFailedDVal = OpenFailed.getAs<DefinedSVal>()) {
    ProgramStateRef StateOpenFailed =
        C.getConstraintManager().assume(State, *OpenFailedDVal, true);
    StateOpenFailed = StateOpenFailed->set<StreamMap>(
        RetSym, StreamState::getOpenFailed(Desc));
    C.addTransition(StateOpenFailed);
  }

  SVal Opened =
      C.getSValBuilder().evalBinOp(State, BO_GT, RetVal, Error, CE->getType());
  if (Optional<DefinedSVal> OpenedDVal = Opened.getAs<DefinedSVal>()) {
    ProgramStateRef StateOpened =
        C.getConstraintManager().assume(State, *OpenedDVal, true);
    StateOpened =
        StateOpened->set<StreamMap>(RetSym, StreamState::getOpened(Desc));
    C.addTransition(StateOpened,
                    constructNoteTag(C, RetSym, "Stream opened here"));
  }
}
