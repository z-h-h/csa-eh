...
void ExprEngine::processCallExit(ExplodedNode *CEBNode) {
  ...
  if (state->hasUncaughtThrow()) {
    ExplodedNodeSet CleanedNodes;
    if (LastSt && Blk && AMgr.options.AnalysisPurgeOpt != PurgeNone) {
      NodeBuilderContext Ctx(getCoreEngine(), Blk, CEBNode);
      currBldrCtx = &Ctx;
      removeDead(CEBNode, CleanedNodes, nullptr, calleeCtx,
    ...
    for (ExplodedNodeSet::iterator I = CleanedNodes.begin(),
      ...
      if (!isNew)
        return;
      Engine.getWorkList()->enqueue(
          CEENode, calleeCtx->getCallSiteBlock(),
          calleeCtx->getIndex()); // Don't increase index
    }
