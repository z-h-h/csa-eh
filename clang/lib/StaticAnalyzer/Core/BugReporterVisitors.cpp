...
//===----------------------------------------------------------------------===//
// Implementation of ExceptionVisitor.
//===----------------------------------------------------------------------===//

void ExceptionVisitor::Profile(llvm::FoldingSetNodeID &ID) const {
  static int Tag = 0;
  ID.AddPointer(&Tag);
}

PathDiagnosticPieceRef ExceptionVisitor::VisitNode(const ExplodedNode *N,
                                                   BugReporterContext &BRC,
                                                   PathSensitiveBugReport &R) {
  auto PS = N->getLocation().getAs<PostStmt>();
  if (!PS)
    return nullptr;
  const Stmt *S = PS->getStmt();
  if (!isa<CXXCatchStmt>(S) && !isa<CXXThrowExpr>(S))
    return nullptr;
  PathDiagnosticLocation L = PathDiagnosticLocation::createBegin(
      S, BRC.getSourceManager(), N->getLocationContext());
  std::string Msg;
  if (isa<CXXCatchStmt>(S))
    Msg = "Catching exception";
  else if (dyn_cast<CXXThrowExpr>(S)->getSubExpr())
    Msg = "Throwing exception";
  else
    Msg = "Re-throwing exception";
  auto P = std::make_shared<PathDiagnosticEventPiece>(L, Msg);
  P->setPrunable(false);
  return P;
}