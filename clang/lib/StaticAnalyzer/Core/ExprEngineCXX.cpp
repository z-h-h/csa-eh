...
void ExprEngine::VisitCXXThrowExpr(const CXXThrowExpr *CTE, ExplodedNode *Pred,
                                   ExplodedNodeSet &Dst) {
  CFGStmt Stmt = getCurrentCFGElement().castAs<CFGStmt>();
  const CFGBlock *TryBlock = Stmt.getTryBlock();
  const LocationContext *LC = Pred->getLocationContext();
  ProgramStateRef State = Pred->getState()->setThrowExpr(CTE);
  if (const auto *SubExpr = CTE->getSubExpr()) {
    // Normal throw
    SVal ThrownVal = State->getSVal(SubExpr, LC);
    State = State->pushThrownVal(ThrownVal, SubExpr->getType());
  } else if (State->getCaughtVals().empty()) {
    // Re-throw & no exception currently being caught, terminate
    StmtNodeBuilder Bldr(Pred, Dst, *currBldrCtx);
    Bldr.generateSink(CTE, Pred, State);
    return;
  } else {
    // Re-throw
    auto CaughtVal = State->getCaughtVals().back();
    State = State->pushThrownVal(CaughtVal.first, CaughtVal.second);
  }
  if (TryBlock != &Pred->getCFG().getExit())
    State = State->setTryBlockIdx(Stmt.getDtorIdx());
  else
    State = State->setUncaughtThrow(true);
  StmtNodeBuilder Bldr(Pred, Dst, *currBldrCtx);
...
void ExprEngine::VisitCXXCatchStmt(const CXXCatchStmt *CS, ExplodedNode *Pred,
  ...
  const LocationContext *LCtx = Pred->getLocationContext();
  ProgramStateRef State = Pred->getState();
  auto ThrownVal = State->getThrownVals().back();
  State = State->popThrownVal();
  State = State->pushCaughtVal(ThrownVal.first, ThrownVal.second);
  ...
  if (VD->getType()->isReferenceType()) {
    Loc LocRefVal = State->getLValue(VD, LCtx);
    MemRegionManager &MRMgr = svalBuilder.getRegionManager();
    const MemRegion *R = MRMgr.getCXXCaughtVarRegion(VD, LCtx);
    Loc LocDerefVal = svalBuilder.makeLoc(R);
    State = State->bindLoc(LocDerefVal, ThrownVal.first, LCtx);
    State = State->bindLoc(LocRefVal, LocDerefVal, LCtx);
  } else {
    State = State->bindLoc(State->getLValue(VD, LCtx), ThrownVal.first, LCtx);
  }
