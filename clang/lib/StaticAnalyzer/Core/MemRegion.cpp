...
void CXXCaughtVarRegion::ProfileRegion(llvm::FoldingSetNodeID &ID,
                                       const VarDecl *D,
                                       const MemRegion *sReg) {
  ID.AddInteger(static_cast<unsigned>(CXXCaughtVarRegionKind));
  ID.AddPointer(D);
  ID.AddPointer(sReg);
}

void CXXCaughtVarRegion::Profile(llvm::FoldingSetNodeID &ID) const {
  ProfileRegion(ID, CatchRefDecl, getSuperRegion());
}
...
void CXXCaughtVarRegion::dumpToStream(raw_ostream &os) const {
  if (const IdentifierInfo *ID = CatchRefDecl->getIdentifier())
    os << ID->getName();
  else
    os << "CaughtVarRegion{D" << CatchRefDecl->getID() << '}';
}
...
bool CXXCaughtVarRegion::canPrintPrettyAsExpr() const { return true; }

void CXXCaughtVarRegion::printPrettyAsExpr(raw_ostream &os) const {
  os << getDecl()->getName();
}
...
const CXXCaughtVarRegion *
MemRegionManager::getCXXCaughtVarRegion(const VarDecl *D,
                                        const LocationContext *LC) {
  const StackFrameContext *SFC = LC->getStackFrame();
  assert(SFC);
  return getSubRegion<CXXCaughtVarRegion>(D, getStackLocalsRegion(SFC));
}
...
static RegionOffset calculateOffset(const MemRegion *R) {
  ...
  while (true) {
    ...
    case MemRegion::ObjCStringRegionKind:
    case MemRegion::NonParamVarRegionKind:
    case MemRegion::CXXCaughtVarRegionKind:
    case MemRegion::ParamVarRegionKind:
    case MemRegion::CXXTempObjectRegionKind:
      // Usual base regions.
      goto Finish;
