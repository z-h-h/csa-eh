...
void ExprEngine::removeDead(ExplodedNode *Pred, ExplodedNodeSet &Out,
  ...
  for (auto &V : CleanedState->getThrownVals()) {
    if (SymbolRef Sym = V.first.getAsSymbol())
      SymReaper.markLive(Sym);
    if (const MemRegion *MR =  V.first.getAsRegion())
      SymReaper.markLive(MR);
  }

  for (auto &V : CleanedState->getCaughtVals()) {
    if (SymbolRef Sym = V.first.getAsSymbol())
      SymReaper.markLive(Sym);
    if (const MemRegion *MR = V.first.getAsRegion())
      SymReaper.markLive(MR);
  ...
  switch (S->getStmtClass()) {
    ...
    case Stmt::CXXCatchStmtClass: {
      ...
      for (const auto Node : PreVisit)
        VisitCXXCatchStmt(cast<CXXCatchStmt>(S), Node, PostVisit);
    ...
    case Stmt::CXXThrowExprClass: {
      ...
      ExplodedNodeSet PostVisit;
      for (const auto Node : PreVisit)
        VisitCXXThrowExpr(cast<CXXThrowExpr>(S), Node, PostVisit);
      getCheckerManager().runCheckersForPostStmt(Dst, PostVisit, S, *this);
      Bldr.addNodes(Dst);
      break;
    }
