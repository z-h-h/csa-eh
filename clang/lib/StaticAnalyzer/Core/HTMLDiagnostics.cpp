...
void HTMLDiagnostics::FinalizeHTML(const PathDiagnostic& D, Rewriter &R,
  ...
  // Add the name of the file as an <h1> tag.
  {
    std::string s;
    llvm::raw_string_ostream os(s);
    std::string BugSummary = "Bug Summary";
    if (D.CTE) {
      SourceLocation Loc = D.CTE->getThrowLoc();
      int L = SMgr.getExpansionLineNumber(Loc);
      int C = SMgr.getExpansionColumnNumber(Loc);
      auto F = llvm::sys::path::filename(SMgr.getFilename(Loc)).str();
      std::ostringstream OSS;
      OSS << BugSummary << " [throw at " << F << ":" << L << ":" << C << "]";
      BugSummary = OSS.str();
    }
