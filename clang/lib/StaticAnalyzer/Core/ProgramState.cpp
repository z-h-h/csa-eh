...
ProgramState::ProgramState(const ProgramState &RHS)
    : llvm::FoldingSetNode(),
      ...
      GDM(RHS.GDM),
      refCount(0),
      hasUncaught(RHS.hasUncaught),
      tryBlockBeginIdx(RHS.tryBlockBeginIdx),
      thrownVals(RHS.thrownVals),
      caughtVals(RHS.caughtVals),
      CTE(RHS.CTE) {
  stateMgr->getStoreManager().incrementReferenceCount(store);
}
...
ProgramStateRef ProgramState::setUncaughtThrow(bool U) const {
  ProgramState newState = *this;
  newState.hasUncaught = U;
  return getStateManager().getPersistentState(newState);
}

ProgramStateRef ProgramState::pushThrownVal(SVal V, QualType T) const {
  ProgramState newState = *this;
  newState.thrownVals.push_back(std::make_pair(V, T));
  return getStateManager().getPersistentState(newState);
}

ProgramStateRef ProgramState::popThrownVal() const {
  ProgramState newState = *this;
  newState.thrownVals.pop_back();
  return getStateManager().getPersistentState(newState);
}

ProgramStateRef ProgramState::pushCaughtVal(SVal V, QualType T) const {
  ProgramState newState = *this;
  newState.caughtVals.push_back(std::make_pair(V, T));
  return getStateManager().getPersistentState(newState);
}

ProgramStateRef ProgramState::popCaughtVals(int Cnt) const {
  assert(Cnt > 0 && (int)caughtVals.size() >= Cnt);
  ProgramState newState = *this;
  while (Cnt--)
    newState.caughtVals.pop_back();
  return getStateManager().getPersistentState(newState);
}

ProgramStateRef ProgramState::setTryBlockIdx(int Idx) const {
  ProgramState newState = *this;
  newState.tryBlockBeginIdx = Idx;
  return getStateManager().getPersistentState(newState);
}

ProgramStateRef ProgramState::setThrowExpr(const CXXThrowExpr *CTE) const {
  ProgramState newState = *this;
  newState.CTE = CTE;
  return getStateManager().getPersistentState(newState);
}
