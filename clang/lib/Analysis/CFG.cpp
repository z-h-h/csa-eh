...
static int FindDtorByID(size_t DtorID, const CFGBlock *B) {
  if (!B || !B->isTryBlock() || !DtorID)
    return -1;
  for (int i = 0; i < (int)B->size(); ++i) {
    auto Dtor = (*B)[i].castAs<CFGAutomaticObjDtor>();
    if (DtorID == Dtor.getUniqueID())
      return i;
  }
  return -1;
}
...
public:
  ...
  public:
    ...
    const LocalScope *getScope() const { return Scope; }
    const LocalScope *getPrevScope() const { return Scope->Prev.getScope(); }
  };
...
class CFGBuilder {
  ...
  // If current CFG block is a block in a catch code block, then these store
  // the current catch statement & parent catch statement.
  std::deque<const CXXCatchStmt *> CatchStmts;

  // Since LocalScope is a constant, need a map from LocalScope to DtorID
  // So we can update DtorID without modifying LocalScope
  using ScopeMapTy = llvm::DenseMap<const LocalScope *, size_t>;
  ScopeMapTy ScopeMap;

  // Record the AODs (automatic object dtor) which have already been appended
  // to the try blocks. Due to return statement, an AOD might be appended to
  // the try blocks multiple times.
  using AppendedDtorsTy = llvm::DenseSet<const VarDecl *>;
  AppendedDtorsTy AppendedDtors;
...
private:
  ...
  void appendStmt(CFGBlock *B, const Stmt *S) {
    ...
    bool NeedUpdateDtorID = false;
    size_t CurDtorID = ScopeMap[ScopePos.getScope()];
    if (const DeclStmt *DS = dyn_cast_or_null<DeclStmt>(S))
      if (reinterpret_cast<size_t>(DS->getSingleDecl()) == CurDtorID)
        NeedUpdateDtorID = true;

    if (NeedUpdateDtorID) {
      if (CurDtorID ==
          reinterpret_cast<size_t>(ScopePos.getFirstVarInScope())) {
        // Update to DtorID of previous scope
        ScopeMap[ScopePos.getScope()] = ScopeMap[ScopePos.getPrevScope()];
      } else {
        // Update to DtorID of next dtor
        int pos = FindDtorByID(CurDtorID, TryTerminatedBlock);
        assert(pos != -1 && pos + 1 < (int)TryTerminatedBlock->size());
        CFGElement E = (*TryTerminatedBlock)[pos + 1];
        CFGAutomaticObjDtor NextDtor = E.castAs<CFGAutomaticObjDtor>();
        ScopeMap[ScopePos.getScope()] = NextDtor.getUniqueID();
      }
    }

    auto Target = TryTerminatedBlock ? TryTerminatedBlock : &cfg->getExit();
    B->appendStmt(const_cast<Stmt *>(S), cfg->getBumpVectorContext(),
                  Target, ScopeMap[ScopePos.getScope()]);
  }

  void appendConstructor(CFGBlock *B, CXXConstructExpr *CE) {
    auto Target = TryTerminatedBlock ? TryTerminatedBlock : &cfg->getExit();
    size_t DtorID = ScopeMap[ScopePos.getScope()];
    if (const ConstructionContext *CC =
            retrieveAndCleanupConstructionContext(CE)) {
      B->appendConstructor(CE, CC, cfg->getBumpVectorContext(), Target, DtorID);
      return;
    }

    // No valid construction context found. Fall back to statement.
    B->appendStmt(CE, cfg->getBumpVectorContext(), Target, DtorID);
  }
  ...
  void appendCall(CFGBlock *B, CallExpr *CE) {
    ...
    if (const ConstructionContext *CC =
            retrieveAndCleanupConstructionContext(CE)) {
      B->appendCXXRecordTypedCall(CE, CC, cfg->getBumpVectorContext(), Target,
                                  DtorID);
      return;
    }
  ...
  void appendObjCMessage(CFGBlock *B, ObjCMessageExpr *ME) {
    ...
    if (const ConstructionContext *CC =
      ...
      B->appendCXXRecordTypedCall(ME, CC, cfg->getBumpVectorContext(), Target,
                                  DtorID);
      return;
    }

    B->appendStmt(const_cast<ObjCMessageExpr *>(ME),
                  cfg->getBumpVectorContext(), Target, DtorID);
  }
  ...
  void appendAutomaticObjDtor(CFGBlock *B, VarDecl *VD, Stmt *S) {
    ...
    if (AppendedDtors.contains(VD))
      return;
    if (!TryTerminatedBlock)
      TryTerminatedBlock = cfg->initPseudoTryBlock();
    size_t DtorID;
    if (VD == ScopePos.getFirstVarInScope()) {
      // DtorID is the ID of prev scope's DtorID (or null if no prev scope)
      DtorID = ScopeMap[ScopePos.getPrevScope()];
    } else {
      // DtorID is the ID of current dtor
      DtorID = ScopeMap[ScopePos.getScope()];
    }
    TryTerminatedBlock->appendAutomaticObjDtor(
        VD, S, cfg->getBumpVectorContext(), DtorID);
    ScopeMap[ScopePos.getScope()] = reinterpret_cast<size_t>(VD);
    AppendedDtors.insert(VD);
  }
...
std::unique_ptr<CFG> CFGBuilder::buildCFG(const Decl *D, Stmt *Statement) {
  ...
  ScopeMap[nullptr] = 0; // no local scope, no dtor
...
CFGBlock *CFGBuilder::VisitCXXThrowExpr(CXXThrowExpr *T) {
  ...
  findConstructionContexts(
      ConstructionContextLayer::create(cfg->getBumpVectorContext(), T),
      T->getSubExpr());
...
CFGBlock *CFGBuilder::VisitCXXTryStmt(CXXTryStmt *Terminator) {
  ...
  size_t DtorID = ScopeMap[ScopePos.getScope()];
  NewTryTerminatedBlock->setTerminator(
      CFGTerminator(Terminator, NewTryTerminatedBlock, DtorID));
...
CFGBlock *CFG::initPseudoTryBlock() {
  PseudoTryBlock = getAllocator().Allocate<CFGBlock>();
  new (PseudoTryBlock) CFGBlock(0xffffffff, BlkBVC, this);
  return PseudoTryBlock;
}

void CFG::finishPseudoTryBlock() {
  assert(PseudoTryBlock && Entry);
  PseudoTryBlock->BlockID = NumBlockIDs++;
  PseudoTryBlock->setHasNoReturnElement();
  PseudoTryBlock->addSuccessor(CFGBlock::AdjacentBlock(Exit, true), BlkBVC);
  Blocks.push_back(PseudoTryBlock, BlkBVC);
}
...
void CFG::calcMergedAndUnmergedBlocks() {
  for (const CFGBlock *B : nodes()) {
    if (B->isTryBlock()) {
      NumUnmergedBlocks += B->size();
      NumMergedBlocks += B->size();
      continue;
    }
    const CFGBlock *prevTryBlock = nullptr;
    for (unsigned i = 0; i + 1 < B->size(); i++) {
      const CFGElement &E = (*B)[i];
      // Filter out non exception statement
      auto CS = E.getAs<CFGStmt>();
      if (!CS)
        continue;
      auto stmt = CS->getStmt();
      if (!dyn_cast_or_null<CallExpr>(stmt) &&
          !dyn_cast_or_null<CXXConstructExpr>(stmt) &&
          !dyn_cast_or_null<CXXThrowExpr>(stmt))
        continue;
      // Every non-last exception statement increases unmerged blocks by 1
      NumUnmergedBlocks++;
      // Evrey time there's a different exception block, increase merged blocks
      // by 1
      if (!prevTryBlock)
        prevTryBlock = CS->getTryBlock();
      if (CS->getTryBlock() && prevTryBlock != CS->getTryBlock()) {
        prevTryBlock = CS->getTryBlock();
        NumMergedBlocks++;
      }
    }
  }
  NumUnmergedBlocks += NumBlockIDs;
  NumMergedBlocks += NumBlockIDs;
}
...
static void print_construction_context(raw_ostream &OS,
  ...
  case ConstructionContext::SimpleThrownValueKind: {
    const auto *TECC = cast<SimpleThrownValueConstructionContext>(CC);
    Stmts.push_back(TECC->getCXXThrowExpr());
    break;
  }
...
static void print_elem(raw_ostream &OS, StmtPrinterHelper &Helper,
                       const CFGElement &E, bool showException);

void CFGElement::dumpToStream(llvm::raw_ostream &OS) const {
  StmtPrinterHelper Helper(nullptr, {});
  print_elem(OS, Helper, *this, false);
}

int CFGStmt::getDtorIdx() const {
  return FindDtorByID(DtorID, TryBlock);
}

int CFGAutomaticObjDtor::getNextDtorIdx() const {
  return FindDtorByID(DtorID, TryBlock);
}

int CFGTerminator::getNextDtorIdx() const {
  return FindDtorByID(NextDtorID, *TryBlock->succ_rbegin());
}

static void print_stmt_next_dtor(raw_ostream &OS, const CFGStmt &Stmt) {
  const CFGBlock *B = Stmt.getTryBlock();
  if (B == &B->getParent()->getExit()) {
    OS << "<EXIT> ";
    return;
  }
  OS << "<B" << B->getBlockID() << ".";
  int idx = Stmt.getDtorIdx();
  if (idx == -1) {
    OS << "T> ";
  } else {
    OS << idx + 1 << "> ";
...
static void print_elem(raw_ostream &OS, StmtPrinterHelper &Helper,
  ...
  case CFGElement::Kind::AutomaticObjectDtor: {
    ...
    if (DE.isExceptionalDtor() && showException) {
      int idx = DE.getNextDtorIdx();
      if (idx == -1) {
        OS << "<T> ";
      } else {
        OS << "<" << idx + 1 << "> ";
...
static void print_block(raw_ostream &OS, const CFG* cfg,
                        ...
                        bool ShowColors, bool showException) {
  ...
  if (&B == &cfg->getEntry())
    ...
    OS << " (EXIT)]";
  else if (&B == cfg->getPseudoTryBlock())
    OS << " (PSEUDO TRY)]";
  ...
  if (showException) {
    const auto &Stmts = B.getCatchStmts();
    PrintingPolicy PP(Helper.getLangOpts());
    for (auto CS : Stmts) {
      OS << " (";
      if (CS->getExceptionDecl())
        CS->getExceptionDecl()->print(OS, PP, 0);
      else
        OS << "...";
      OS << ")";
    }
  ...
  for (CFGBlock::const_iterator I = B.begin(), E = B.end() ;
    ...
    print_elem(OS, Helper, *I, showException);
  }

  // Print the terminator of this block.
  if (&B == cfg->getPseudoTryBlock()) {
    if (ShowColors)
  ...
  if (print_edges) {
    ...
    if (!B.succ_empty()) {
      ...
      for (CFGBlock::const_succ_iterator I = B.succ_begin(), E = B.succ_end();
        ...
        if (!Succ_B) {
          Reachable = false;
          Succ_B = I->getPossiblyUnreachableBlock();
        }

        if (Succ_B) {
          OS << " B" << Succ_B->getBlockID();
          if (showException && Succ_B->isTryBlock() && B.isTryBlock()) {
            assert(Succ_B == *B.succ_rbegin());
            int idx = B.getTerminator().getNextDtorIdx();
            if (idx != -1) {
              OS << "." << idx + 1;
            } else {
              OS << ".T";
...
void CFG::dump(const LangOptions &LO, bool ShowColors) const {
  print(llvm::errs(), LO, ShowColors, false);
}

/// dumpException - A simple pretty printer of a CFG (with exception
/// information) that outputs to stderr.
void CFG::dumpException(const LangOptions &LO, bool ShowColors) const {
  print(llvm::errs(), LO, ShowColors, true);
}

/// print - A simple pretty printer of a CFG that outputs to an ostream.
void CFG::print(raw_ostream &OS, const LangOptions &LO, bool ShowColors,
                bool showException) const {
  StmtPrinterHelper Helper(this, LO);

  // Print the entry block.
  print_block(OS, this, getEntry(), Helper, true, ShowColors, showException);
  ...
  for (const_iterator I = Blocks.begin(), E = Blocks.end() ; I != E ; ++I) {
    ...
    // Skip the pesudo try block
    if (*I == getPseudoTryBlock() && !showException)
      continue;

    print_block(OS, this, **I, Helper, true, ShowColors, showException);
  }

  // Print the exit block.
  print_block(OS, this, getExit(), Helper, true, ShowColors, showException);
  OS << '\n';
  OS.flush();
}
...
void CFGBlock::print(raw_ostream &OS, const CFG* cfg,
  ...
  print_block(OS, cfg, *this, Helper, true, ShowColors, false);
  OS << '\n';
}
...
bool CFGBlock::isTryBlock() const {
  return (this == getParent()->getPseudoTryBlock()) ||
         (getTerminatorStmt() && isa<CXXTryStmt>(getTerminatorStmt()));
}
...
struct DOTGraphTraits<const CFG*> : public DefaultDOTGraphTraits {
  ...
  static std::string getNodeLabel(const CFGBlock *Node, const CFG* Graph) {
    ...
    print_block(Out,Graph, *Node, *GraphHelper, false, false, false);
