...
LiveVariables::computeLiveness(AnalysisDeclContext &AC, bool killAtAssign) {
  ...
  while (const CFGBlock *block = worklist.dequeue()) {
    ...
    // Merge the try blocks (try blocks are treated as successors)
    llvm::BitVector MergedTryBlocks(cfg->getNumBlockIDs());
    for (auto I = block->begin(); I != block->end(); ++I) {
      auto CS = I->getAs<CFGStmt>();
      if (!CS)
        continue;
      const CFGBlock *Try = CS->getTryBlock();
      if (!everAnalyzedBlock[Try->getBlockID()]) {
        // FIXME: should recursively compute 'Try' block
        continue;
      }
      if (!MergedTryBlocks[Try->getBlockID()]) {
        val = LV->merge(val, LV->blocksBeginToLiveness[Try]);
        MergedTryBlocks[Try->getBlockID()] = true;
      }
    }
