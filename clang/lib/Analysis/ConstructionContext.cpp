...
const ConstructionContext *ConstructionContext::createBoundTemporaryFromLayers(
  ...
  case ConstructionContextItem::ThrowKind: {
    const auto *TE = cast<CXXThrowExpr>(ParentItem.getStmt());
    return create<SimpleThrownValueConstructionContext>(C, TE);
  }
...
const ConstructionContext *ConstructionContext::createFromLayers(
  ...
  case ConstructionContextItem::ThrowKind: {
    assert(TopLayer->isLast());
    const auto *TE = cast<CXXThrowExpr>(TopItem.getStmt());
    return create<SimpleThrownValueConstructionContext>(C, TE);
  }
  case ConstructionContextItem::MaterializationKind: {
