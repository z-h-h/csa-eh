...
static cl::opt<bool>
    EmitAST("emit-ast",
            cl::desc(Options.getOptionHelpText(options::OPT_emit_ast)),
            cl::cat(ClangCheckCategory));
static cl::opt<bool>
    ASTDump("ast-dump",
...
int main(int argc, const char **argv) {
  ...
  if (!Output.getValue().empty()) {
    ...
    Tool.appendArgumentsAdjuster(getInsertArgumentAdjuster(Output.getValue().c_str()));
  }
  ...
  if (Analyze)
    ...
    FrontendFactory = newFrontendActionFactory<clang::GeneratePCHAction>();
  else if (Fixit)
